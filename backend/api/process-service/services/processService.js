...
export class ProcessService {
  ...
  `);
    ...
    if (data_inicial && data_final) {
      ...
      whereParts.push(
        `data_disponibilizacao BETWEEN $${paramsNormalizaed.length - 1} AND $${
          paramsNormalizaed.length
        }`
      );
  ...
  async updateStatus(hash_id, newStatus) {
    const process = await prisma.processo.findUnique({
